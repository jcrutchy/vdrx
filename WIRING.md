...
`TVDRX_SocketListenerExecutive` (`vdrx_socketlistener.pas`) instead of each
...
```pascal
uses
  ..., vdrx_core, vdrx_socketlistener, vdrx_config, vdrx_admin, vdrx_logger,
  vdrx_bridge, vdrx_whiteboard, vdrx_irc, vdrx_websocket, vdrx_http;
...
var
  ...
  // Bridge: TVDRX_BridgeExecutive;  -- only if you have an external process to run tonight
...
begin
  ...
  WS := TVDRX_WebSocketExecutive.Create(Kernel.Queue, Kernel.Registry);
  Kernel.Registry.Register(WS, 'websocket', 'sys.none');   // binds its OWN port (8082)

  HTTP := TVDRX_HTTPExecutive.Create(Kernel.Queue, Whiteboard);
...
Use `TVDRX_WebListenerExecutive` (`vdrx_weblistener.pas`) instead of registering `HTTP`
and `WS` as their own listeners. `WS` still needs to exist - `TVDRX_WSConnection`
...
var
  Web: TVDRX_WebListenerExecutive;
...
begin
  ...
  // to give TVDRX_WebListenerExecutive somewhere to hand upgraded connections off
  // to (AdoptConnection), and to give TVDRX_WSConnection instances Bus/Registry.
  WS := TVDRX_WebSocketExecutive.Create(Kernel.Queue, Kernel.Registry);

  Web := TVDRX_WebListenerExecutive.Create(Kernel.Queue, Whiteboard, WS);
  Web.Port := 80;
...
dynamically-registered children (`TVDRX_WSConnection`) or their reader threads
...
as the message text (or extend `vdrx_irc`'s `PRIVMSG` handler to translate a `!wb`
...
`vdrx_daemon.conf` and re-applies it to every registered executive via their
`ApplyConfig` hook.
...
- RBAC on `sys.*`/`admin.*`
- Per-connection thread tracking in `TVDRX_SocketListenerExecutive.Shutdown`
...
- Decoupling `TVDRX_WSConnection.HandlePacket` from the Kernel's dispatch thread via
...
`vdrx_socketlistener.pas`'s `HandleConnection` now takes a `TVDRX_Transport`
(see new unit `vdrx_transport.pas`) instead of a raw `TSocket`. Every protocol
executive (`vdrx_http.pas`, `vdrx_websocket.pas`, `vdrx_irc.pas`,
`vdrx_weblistener.pas`) reads/writes through `ATransport.Read`/`.Write`/`.Close`
instead of `fpRecv`/`fpSend`/`CloseSocket` directly, so none of them know or care
whether a connection is plaintext or TLS.

`TVDRX_SocketListenerExecutive` can now bind a plain port and a TLS port at the
...
Config keys per socket-listener executive (see `vdrx_daemon.conf`):
`port`, `tls_port` (0 = disabled), `tls_cert`, `tls_key` (absolute paths). IRCD is
wired up in `vdrx_daemon.lpr`; HTTP/WS classes support the same keys
(`executives.http.*` / `executives.ws.*`) and their own `ApplyConfig`, but aren't
instantiated in `vdrx_daemon.lpr` yet - marked `"enabled": false` in the conf as a
...
1. FPC's bundled `openssl` unit (`/usr/share/fpcsrc/<ver>/packages/openssl/src/openssl.pas`)
   ...
   `vdrx_transport.pas` now uses the correct names throughout.
...
`TVDRX_SocketListenerExecutive.TLSActive` reflects real go-live status now (TLS
port configured AND cert/key loaded AND accept loop started), separate from
`TLSPort` which just reflects what was asked for - `vdrx_daemon.lpr`'s startup
